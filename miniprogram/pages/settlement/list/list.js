...
Page({
  data: {
    ...
    statusClass: { SEALED: 'tag-success', APPROVED: 'tag-primary', PENDING: 'tag-warning', REJECTED: 'tag-danger', DRAFT: 'tag-info' },
    showForm: false,
    completedProjects: [],
  ...
  async loadProjects() {
    const user = app.globalData.userInfo
    const res = await api.getProjects({ leaderId: user.id, status: 'COMPLETED', size: 100 })
    this.setData({ completedProjects: res.data.records })
  },
  onProjectChange(e) {
    const idx = e.detail.value
    this.setData({ projectIndex: idx, 'form.projectId': this.data.completedProjects[idx].id })
  },
  onInput(e) {
  ...
  async handleReview(e) {
    ...
    const { confirm } = await wx.showModal({ title: '确认', content: `确定${actionText}此结算？` })
    if (!confirm) return
    try {
      await api.reviewSettlement({ id, action, comment: actionText })
  ...
  async handleSeal(e) {
    ...
    const { confirm } = await wx.showModal({ title: '确认', content: '确定为此结算盖章？' })
    if (!confirm) return
    try {
      await api.sealSettlement(id)
  ...
  async submitForm() {
    ...
    this.setData({ showForm: false, form: { projectId: '', equipmentFee: '', materialFee: '', travelFee: '', meetingFee: '', laborFee: '', consultFee: '', otherFee: '', remark: '' }, projectIndex: -1 })
    this.loadData()
  }
})
